...
- 保留原文中的超链接。
- 图片使用 node 脚本 `.Codex/skills/fetch-tutorial-images/download-image.js` 下载（从浏览器 evaluate 输出的 base64 结果文件中提取）。
...
4. **逐个下载图片**：每轮 3 张图片——
   ...
   - 再逐个调用 `node .Codex/skills/fetch-tutorial-images/download-image.js` 保存这 3 张（分别调用，不得使用任何批量处理参数，避免反复请求权限）。
   - 重复至全部下载完。重复 URL 的图片（同一张图被引用多次）只下载一次，后续跳过。
5. 运行 `node .Codex/skills/fetch-tutorial-images/download-image.js <临时文件> <目标图片路径>` 保存图片。
...
- `download-image.js` 脚本路径统一用 `.Codex/skills/fetch-tutorial-images/download-image.js`（相对于项目根目录）。
