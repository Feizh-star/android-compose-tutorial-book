...
description: >
  ...
  Trigger at the end of each task to alert the user that Claude has finished responding.
...
Sends a Windows native toast notification (or popup fallback) when invoked. Use it at the end of each task so the user knows Claude has finished responding without having to watch the window.
  ...
  "Stop": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "powershell -ExecutionPolicy Bypass -File \"f:/Android/note/.claude/skills/notify/notify.ps1\""
        }
      ]
    }
  ]
