...
Options:
  ...
  --proxy-server <url>       Proxy for Playwright browser/request, e.g. http://127.0.0.1:7890.
  --no-system-proxy          Do not auto-detect env/Windows system proxy.
...
async function discoverWithBrowser(html, baseUrl, timeout, proxyServer) {
...
function normalizeProxyServer(value) {
  ...
  if (!proxy) return "";
  if (/^(https?|socks4|socks5):\/\//i.test(proxy)) return proxy;
  return `http://${proxy}`;
}

function pickProxyFromWindowsProxyServer(value) {
  if (!value) return "";
  ...
  for (const item of proxy.split(";")) {
    ...
    parts.set(rawKey.trim().toLowerCase(), rawValue.join("=").trim());
  }
  return parts.get("https") || parts.get("http") || parts.get("socks") || "";
}
...
async function detectWindowsSystemProxy() {
  ...
  try {
    const enabled = await execFilePromise("reg.exe", ["query", key, "/v", "ProxyEnable"], {
      windowsHide: true,
      timeout: 3000,
    });
    if (!/\b0x1\b/i.test(enabled.stdout)) return "";

    const server = await execFilePromise("reg.exe", ["query", key, "/v", "ProxyServer"], {
      windowsHide: true,
      timeout: 3000,
    });
    const match = server.stdout.match(/ProxyServer\s+REG_\w+\s+(.+)/i);
    return normalizeProxyServer(pickProxyFromWindowsProxyServer(match && match[1]));
  } catch {
    return "";
  }
}

async function resolveProxyServer(args) {
  if (args.proxyServer) return normalizeProxyServer(args.proxyServer);
  if (args.noSystemProxy) return "";
  ...
  const envProxy = process.env.HTTPS_PROXY
    ...
    || process.env.all_proxy;
  return normalizeProxyServer(envProxy) || await detectWindowsSystemProxy();
}
...
async function downloadAll(initialUrls, baseUrl, assetsDir, concurrency, timeout, retries, dryRun, refresh, preloadedAssets, proxyServer) {
...
async function main() {
  ...
  const proxyServer = await resolveProxyServer(args);
  ...
  const browserDiscovery = await discoverWithBrowser(html, baseUrl, args.timeout, proxyServer).catch((error) => {
